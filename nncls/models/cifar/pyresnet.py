...
class PyConvBasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, pyconv_groups=1, pyconv_kernels=1):
        super().__init__()

        #residual function
        self.residual_function = nn.Sequential(
            get_pyconv(in_channels, out_channels, pyconv_kernels=pyconv_kernels, stride=stride,
...
class PyConvBottleNeck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, pyconv_groups=1, pyconv_kernels=1):
        ...
        self.residual_function = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            get_pyconv(out_channels, out_channels, pyconv_kernels=pyconv_kernels, stride=stride, pyconv_groups=pyconv_groups),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels, out_channels * PyConvBottleNeck.expansion, kernel_size=1, bias=False),
            nn.BatchNorm2d(out_channels * PyConvBottleNeck.expansion),
        )

        self.shortcut = nn.Sequential()

        if stride != 1 or in_channels != out_channels * PyConvBottleNeck.expansion:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, out_channels * PyConvBottleNeck.expansion, stride=stride, kernel_size=1, bias=False),
                nn.BatchNorm2d(out_channels * PyConvBottleNeck.expansion)
            )
...
class PyConvResNet(nn.Module):
    ...
    def _make_layer(self, block, out_channels, num_blocks, stride, pyconv_kernels=[3], pyconv_groups=[1]):
        ...
        for stride in strides:
            layers.append(block(self.in_channels, out_channels, stride, pyconv_kernels=pyconv_kernels, pyconv_groups=pyconv_groups))
...
def pyconvresnet18():
    ...
    """
    return PyConvResNet(PyConvBasicBlock, [2, 2, 2, 2])

def pyconvresnet34():
    """ return a ResNet 34 object
    """
    return PyConvResNet(PyConvBasicBlock, [3, 4, 6, 3])

def pyconvresnet50():
    """ return a ResNet 50 object
    """
    return PyConvResNet(PyConvBottleNeck, [3, 4, 6, 3])

def pyconvresnet101():
    """ return a ResNet 101 object
    """
    return PyConvResNet(PyConvBottleNeck, [3, 4, 23, 3])

def pyconvresnet152():
    """ return a ResNet 152 object
    """
    return PyConvResNet(PyConvBottleNeck, [3, 8, 36, 3])
# class PyConvResNet(nn.Module):

#     def __init__(self, block, layers, num_classes=100, zero_init_residual=False, norm_layer=None, dropout_prob0=0.0):
...
#     model = PyConvResNet(PyConvBasicBlock, [2, 2, 2, 2], **kwargs)  #params=5.91M GFLOPs 0.88
#     if pretrained:
...
#     model = PyConvResNet(PyConvBasicBlock, [3, 4, 6, 3], **kwargs)  #params=11.09M GFLOPs 1.75
#     if pretrained:
#         raise NotImplementedError("Not available the pretrained model yet!")
...
#         pretrained (bool): If True, returns a model pre-trained on ImageNet
#     """
#     model = PyConvResNet(PyConvBottleNeck, [3, 4, 23, 3], **kwargs)
#     if pretrained:
#         os.makedirs(default_cache_path, exist_ok=True)
...
#         pretrained (bool): If True, returns a model pre-trained on ImageNet
#     """
#     model = PyConvResNet(PyConvBottleNeck, [3, 8, 36, 3], **kwargs)
#     if pretrained:
#         os.makedirs(default_cache_path, exist_ok=True)
